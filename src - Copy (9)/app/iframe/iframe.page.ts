...
export class IframePage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.userbalance = +this.getParameterByName('balance');
    console.log("in iframe.ts");
  ...
  getcryptograph(abbv:string,id:number,abbvgraph:string) {
   console.log(abbv,id,abbvgraph);
    ...
    // $(document).ready(function () {
      ...
      var arr1 = [];
      var dataCount;
      this.http.get(`https://api.polygon.io/v2/aggs/ticker/${abbv}/range/1/minute/2021-11-11/2021-11-11?adjusted=true&sort=asc&apiKey=ju5mjYL3pwvojO1DjYV3zxdFXUxuHtx6`)
      .subscribe((data)=>{
        history.push(data);
      ...
      const subject = webSocket("wss://socket.polygon.io/crypto");
      subject.next({ action: "auth", params: "ju5mjYL3pwvojO1DjYV3zxdFXUxuHtx6" });
      subject.next({ action: "subscribe", params: "XT."+abbvgraph });
      var val;
  ...
  getcryptograph1(abbv: any) {
    $(document).ready(function () {
      ...
      socket.addEventListener('open', function (event) {
        ...
        socket.send(JSON.stringify({ action: "subscribe", params: "`${abbvgraph}`" }));
